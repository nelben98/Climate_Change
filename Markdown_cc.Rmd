...
plot<-ggplot(elec_prod_clean,aes(x=TIME_PERIOD, y=Consumed/10**6, fill=Fuel)) + 
  ...
  labs(x="Year",y=expression(paste('Energy consumed (',10^6,' GWE)')),title="Cummulative energy production (breaking down electricity)") + theme_bw() +scale_fill_brewer(palette = "RdYlBu")

plot + annotate("label",x = c(rep(as.Date("2020-01-01","%Y-%m-%d"),6)),
   y = c(9.8,6.5,4.5,2.5,1,0.2),
   label = paste(round(preserving_orders$prop.x,3)*100,"%"), color="black", 
           size=2.5 , fontface="bold") +
  annotate("label",x = c(rep(as.Date("2017-01-01","%Y-%m-%d"),6)),
   y = c(10.2,6.8,4.7,2.2,1,0.2),
   label = paste(sort(round(prop_elec_2017$prop,3)*100,decreasing=TRUE),"%"), color="black", 
           size=2.5 , fontface="bold")
```
...
plot_2<-ggplot(prod_clean,aes(x=TIME_PERIOD, y=Consumed/10**6, fill=Fuel)) + 
  geom_area(alpha=0.6 , size=1, colour="black") + 
  labs(x="Year",y=expression(paste('Energy consumed (',10^6,' GWE)')),title="Cummulative energy production inc. electricity") + theme_bw() +scale_fill_brewer(palette = "RdYlBu") + theme(axis.text.x = element_text(angle = 45,hjust=1))
...
plot_2 + annotate("label",x = c(rep(as.Date("2020-01-01","%Y-%m-%d"),6)),
  ...
  annotate("label",x = c(rep(as.Date("2011-01-01","%Y-%m-%d"),6)),
   y = c(9,5.2,3,1.5,0.6,0),
   label = paste(sort(round(energy_at_2017$prop,3)*100,decreasing=TRUE),"%"), color="black", 
           size=2.5 , fontface="bold")
```
...
ggplot(em_sec_sec,aes(x=Sectors,y=Consumed))+geom_col()+theme(axis.text.x = element_text(angle = 45,hjust=1))
...
plot_3.1<- ggplot(sec_energy_prod %>% filter(Country_code=="EUA") ,aes(x= as.integer(Year), y=Emissions/1000, group=Sector_name,col=Sector_name)) + 
  geom_line() + geom_point(alpha=.5) +  
  labs(x="Years",y=expression(paste('Emissions (',1000,'s Gigagrams)')),title="Cummulative emissions by sector",
       col="Sectors") + theme_bw() + theme(axis.text.x = element_text(angle = 45,hjust=1))
plot_3.1
```
                    ...
                    mutate(Sector_name= as.factor(Sector_name),
                           Sector_name=fct_collapse(Sector_name," Other transports"=
                                                      c("1.A.3.b.iv - Motorcycles",
                                                        ...
                                                        "1.A.3.a - Domestic Aviation",
                                                        "1.A.3.e - Other Transportation"))
                           ) %>% group_by(Country_code,Year,Sector_name) %>% summarise(Emissions = sum(emissions)) %>%  
...
plot_3.2<-ggplot(sec_energy_transport %>% filter(Country_code=="EUA") ,aes(x=as.integer(Year), y=Emissions/1000, group=Sector_name,col=Sector_name)) +
  geom_line() + geom_point(alpha=.5)+
  labs(x="Years",y=expression(paste('Emissions (',1000,'s Gigagrams)')),title="European Cummulative emissions by transport",
       col="Sectors") + theme_bw() + theme(axis.text.x = element_text(angle = 45,hjust=1))
...
plot_3.3 <- ggplot(sec_energy_residential %>%
                 filter(Country_code=="EUA") ,aes(x=as.integer(Year), y=Emissions/1000, group=Sector_name,col=Sector_name)) +
  geom_line() + geom_point(alpha=.5)+
   theme_bw() + theme(axis.text.x = element_text(angle = 45,hjust=1)) +labs(x="Years",y=expression(paste('Emissions (',1000,'s Gigagrams)')),title="European Cummulative emissions by sectors",
       col="Sectors")
...
europe = c((dictionary_countries %>% mutate(Country=ifelse(Country=="United Kingdom (Convention)","United Kingdom",Country),Country_codes=ifelse(Country_codes=="GE","EL",Country_codes)) %>% filter(Country != c("EU-27+UK","EU (KP)","EU-27")))$Country)

europe_codes = c((dictionary_countries %>% mutate(Country=ifelse(Country=="United Kingdom (Convention)","United Kingdom",Country),Country_codes=ifelse(Country_codes=="GE","EL",Country_codes)) %>% filter(Country != c("EU-27+UK","EU (KP)","EU-27")))$Country_codes)


# Extrapolate the data of the UK - repeat last value:
a<-data_prod %>% mutate(Energy=OBS_VALUE/10^4) %>% 
...
fill_pol_en<-left_join(eur_em  %>% mutate(Country = Geographic_entity, Pollution = eur_em$'2020' ) %>% filter( Country != "EU-27+UK",Country != "EU (KP)",Country != "EU-27")%>% select(Country,Pollution),total_energy,by= "Country") %>%  transmute(Country,Pollution=Pollution/Energy,region=Country,Fuel,year=TIME_PERIOD,Pol=Pollution,En=Energy)
...
# 4- FUTURE See the car industry, which is one of the biggest polluters, and see energy investment growth by subtype.
